...
python3 -m py_compile backend/main.py backend/services.py test_api_offline.py tests_services_unit.py
...
if python3 -m venv "$VENV_DIR" >/dev/null 2>&1; then
  ...
  python -m unittest -q test_api_offline.py tests_services_unit.py
  deactivate
else
  ...
  if [[ -f "$APP_DIR/.venv/bin/python" ]]; then
    if "$APP_DIR/.venv/bin/python" -c "import fastapi" >/dev/null 2>&1; then
      "$APP_DIR/.venv/bin/python" -m unittest -q test_api_offline.py tests_services_unit.py
    else
